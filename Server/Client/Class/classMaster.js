...
export const CreateClass = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [autoID, QstClass, newData.CLNAME, newData.SECNAME, Adddate, newData.AddedBy, newData.IsActive]
        );
        if (result.length > 0) {
            res.json({
                data: [
...
export const editClass = async (req, res) => {
    ...
    console.log(id);
    try {
        const result = await pool.query("SELECT * FROM classmaster WHERE classId = ?;", [id]);
        if (result[0].length > 0) {
            res.json({
                data: [
                    {
...
export const updateClass = async (req, res) => {
    ...
    try {
        const result = await pool.query(
            ...
            [QstClass, newData.CLNAME, newData.SECNAME, ModifiedDate, newData.ModifiedBy, newData.IsActive, id]
        );
        console.log(result);
        if (result.length > 0) {
            res.json({
...
export const DeleteClass = async (req, res) => {
    ...
    try {
        const result = await pool.query(`DELETE FROM classmaster WHERE ClassId IN (${id});`);
        console.log(result);
        if (result.length > 0) {
            res.json({
                data: [
