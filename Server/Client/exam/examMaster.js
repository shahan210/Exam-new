...
export const getYearClassSubjectWiseList = async (req, res) => {
  ...
  try {
    ...
    const selectedSubject = await pool.query(
...
export const getExamMasterInfo = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM examdefinitiontmst WHERE QuestionTestID = ?;",
      [id]
    );
    if (result.length > 0) {
      res.json({
...
export const createdQuestionMaster = async (req, res) => {
  ...
  try {
    ...
    const createQuestImage = await pool.query(
      `INSERT INTO questionimages (QuestionBankID) VALUES(?);`,
      [newQuesId]
    );
...
export const getQuizMasterEditInfo = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM questionbankmst WHERE QuestionBankID = ?;",
      [id]
    );
...
export const UploadFileData = async (req, res) => {
  ...
  if (fileExtension !== ".xls" && fileExtension !== ".xlsx") {
    return res
      .status(400)
      .send("Invalid file format. Please upload an Excel file.");
  }
