...
export const getYearClassSubjectWiseList = async (req, res) => {
  const Data = req.body;
  try {
    const result = await pool.query(`SELECT * FROM examdefinitiontmst WHERE AcaYear = ? AND SubjectID = ? AND ClassId = ?;`, [
      Data.year,
      Data.subject.id,
      Data.class.id,
    ]);
    ...
    const examData = result[0];
    const selectedSubjectName = selectedSubject[0][0].SubjectName;
    const selectedClassName = selectedClass[0][0].QstClass;
    const response = examData.map((exam) => ({
      ...exam,
...
export const createdQuestionMaster = async (req, res) => {
  ...
  try {
    ...
    const createQuestImage = await pool.query(
      `INSERT INTO questionimages (QuestionBankID,fileName,QuizTittle,Answer1,Answer2,Answer3,Answer4,pathName) VALUES(?,?,?,?,?,?,?,?);`,
      [newQuesId, "", "", "", "", "", "", ""]
    );
...
export const editExamMasterInfo = async (req, res) => {
  ...
  const updateValues = [
    ...
    formattedDate,
    pa.AddedBy,
    pa.ALTMTHour ? pa.ALTMTHour : 0,
    pa.ALTMTMin ? pa.ALTMTMin : 0,
    pa.ALTMTSec ? pa.ALTMTSec : 0,
    pa.ExamHeading,
    pa.ExamSubHeading,
    pa.QuestionTestID,
  ];
...
export const UpdateExamMark = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("UPDATE questionbankmst SET Mark = ? WHERE QuestionTestID = ?;", [data.Mark, data.QuestionTestID]);
