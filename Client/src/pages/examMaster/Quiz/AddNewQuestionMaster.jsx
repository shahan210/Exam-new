...
import getDownloadBlank from "../../../API/examMaster/getDownloadBlank";
...
const AddNewQuestionMaster = () => {
  ...
  const handleDownloadBlank = async () => {
    try {
      const response = await getDownloadBlank();
    ...
    <Layout>
      <div className="">
        ...
        <div className="w-full flex gap-2">
              ...
              <form>
                <div className="flex w-full max-w-sm items-center gap-1.5">
                  ...
                  {excelsubmit && excelFile && (
                    <Button onClick={hanldeFileUpload} className="bg-green-500 text-white px-4 py-2 mx-auto" type="submit">
                      upload
                    </Button>
                  )}
                </div>
          ...
          <Button type="button" className="my-auto" onClick={handleDownloadBlank}>
            Download Blank Question Model
          </Button>
        </div>
        ...
        <div className=" p-6 rounded-md w-full md:w-3/4 lg:w-full ">
          ...
          <form className="flex flex-col space-y-4">
                ...
                return (
                  <div className="flex flex-col gap-2" key={optionIndex}>
                    <div className="flex flex-col lg:flex-row  lg:space-x-2 items-center justify-between">
                      <Label className="w-28" htmlFor={`option-${optionIndex}`}>{`Option ${optionIndex + 1}`}</Label>
                      {option.img && (
                        <div className="flex justify-start max-w-44 w-fit min-w-32">
                          <div className="flex relative mx-auto">
                            ...
                            <img src={option.imgPreview} alt="lgogogogogog" className="size-20" />
                          </div>
                        </div>
                      )}
                      <div className="w-full flex space-x-2">
                        <div className="justify-between flex-1 space-x-2 items-center flex">
                          ...
                          <Input
                            ...
                            accept="image/*"
                          ></Input>
                          <Button type="button" onClick={() => handleRemoveOption(optionIndex)} variant="destructive">
                            <X className="h-4 w-4" />
                          </Button>
                          <Checkbox
                            className="w-7 h-7"
            ...
            <Button type="submit" className="bg-green-500 text-white px-4 py-2 mt-4 mx-auto" onClick={handleSubmission}>
              Submit
            </Button>
          </form>
          {/* )} */}
